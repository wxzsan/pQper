...
var vm = new Vue({
  ...
  methods: {
    ...
    // 页面初始化，填入所有数据
    initDatas() {
      this.$axios.get('http://39.105.19.68:8000/commentarea/get_create_comment_area_request')
        .then(
          (res) => {
            console.log(res)
            res = res.data
            ...
            res.data.createRequestList.forEach((element) => {
              this.$axios.get('http://39.105.19.68:8000/commentarea/get_paper?paperId=' + element.paper)
                .then(
                  (response) => {
    ...
    // 通过申请事件处理
    handleAccept(count) {
      this.$axios.get('http://39.105.19.68:8000/commentarea/approve_create_comment_area_request?requestId=' + this.createRequestList[count - 1].requestId)
        .then(
          (res) => {
            res = res.data
            if (res.code === 200) {
