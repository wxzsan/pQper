...
var vm = new Vue({
  ...
  methods: {
    ...
    // 页面初始化，填入所有数据
    initDatas() {
      this.$axios.get('http://39.105.19.68:8000/commentarea/get_star_long_comment_list')
        .then(
          (res) => {
            res = res.data
            if (res.code != 200) {
            ...
            res.longCommentList.forEach((longCommentId) => {
              this.$axios.get('http://39.105.19.68:8000/commentarea/get_long_comment?inCommentArea=false&longCommentId=' + longCommentId)
                .then(
                  (response) => {
    ...
    handleDelete(count) {
      if (this.longCommentList[count - 1].is_owner) {
        this.$axios.get('http://39.105.19.68:8000/commentarea/delete_long_comment?longCommentId=' + this.longCommentList[count - 1].id)
          .then(
            (res) => {
              res = res.data
              if (res.code === 200) {
