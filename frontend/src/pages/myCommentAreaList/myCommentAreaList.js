...
var vm = new Vue({
  ...
  methods: {
    ...
    // 页面初始化，填入所有评论数据
    initDatas() {
      this.$axios.get('http://39.105.19.68:8000/commentarea/get_star_comment_area_list')
        .then(
          (res) => {
            res = res.data
            if (res.code != 200) {
            ...
            res.commentAreaList.forEach((commentAreaId) => {
              this.$axios.get('http://39.105.19.68:8000/commentarea/get_comment_area?commentAreaId=' + commentAreaId)
                .then(
                  (response) => {
    ...
    handleStarArea(count) {
      if (!this.commentAreaInfoList[count - 1].area_has_star) {
        this.$axios.get('http://39.105.19.68:8000/commentarea/star_comment_area?commentAreaId=' + this.commentAreaInfoList[count - 1].id)
          .then(
            (res) => {
              if (res.code === 200) {
        ...
        this.$axios.get('http://39.105.19.68:8000/commentarea/cancel_star_comment_area?commentAreaId=' + this.commentAreaInfoList[count - 1].id)
          .then(
            (res) => {
              if (res.code === 200) {
